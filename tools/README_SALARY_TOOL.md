...
The tool reads a `salary_data.json` file in the repo root containing company salary benchmarks. It uses fuzzy matching to find companies by name, handling accented characters, legal suffixes (Inc, Ltd, Corp), and common spelling variations.
...
```json
{
  "metadata": {
    ...
    "index_baseline": 0,
    "index_label": "Annual salary (CAD)",
    "baseline_description": "Approximate annual salary before tax"
  },
  "companies": [
    {
...
```bash
python salary_lookup.py "Shopify"
python salary_lookup.py "Telus" --city "Vancouver"
python salary_lookup.py "RBC" --json
